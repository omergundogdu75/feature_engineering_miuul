...
# !pip install missingno # eksik değerde kullanacağız.
import missingno as msno
from datetime import date
from sklearn.metrics import accuracy_score
from sklearn.model_selection import train_test_split
from sklearn.neighbors import LocalOutlierFactor # çok değişkenli aykırı değer yöntemi LOF
...
###################
# Grafik Teknikle Aykırı Değerler: box plot yöntemi kullanılır.
# elimizde bir sayı değişken değer varsa boxplot dan sonra histogram ile gösterebiliriz.
###################
...
###################
# Aykırı Değerler Nasıl Yakalanır? çeyrek değerlerine erişelimki aykırı değerlere ulaşalım
###################
...
###################
# Aykırı Değer Var mı Yok mu? hızlı şekilde sorarsak
###################
...
outlier_thresholds(df, "Age") # alt ve üst değerler belirlendi.
...
###################
# grab_col_names: burada çok sayıda değişken var çok sayıda değişken olduğundan
# hangisinin sayısaıl, kategorik, kategorik görünümlü sayısal, sayısal olmasada kategorik değişkenleri
# getirmesini bekleri
###################
...
df.shape[0] - new_df.shape[0] # kaç değişiklik silindi.

###################
# Baskılama Yöntemi (re-assignment with thresholds)
# Bir hücredeki
# aykırılıkdan dolayı silme işlemi yapıldığında diğer hücrelerdeki verileri de kaybediyoruz.
# Dolayısıyla silmek yerine baskılamayıda tercih edebiliriz.
# kabul edilebilir bazı eşik değerleri ile bir hücredeki değeri değiştirerek baskılayabiliriz.
#Bu işlem veri kaybetmemek için yaparız.
###################
...
df = load() # veri seti oku
outlier_thresholds(df, "Age") # aykırı değerlerin eşik değerleri sapta
check_outlier(df, "Age") # aykırı değer var mı
grab_outliers(df, "Age", index=True) # aykırı değer indexleri

remove_outlier(df, "Age").shape # silinen değişken sayısı
replace_with_thresholds(df, "Age") # baskılama yöntemi kullan
check_outlier(df, "Age")  # aykırı değer var mı
...
#############################################
# tek başına ele alınınca aykırı olmayıp, çok değişkenli olduğunda aykırı olabilir.
# Lof: Gözlemleri yoğunluk tabanlı skorlayarak,  buna göre aykırı değer tanımı yapar
# Bir noktanın local yoğunluğu demek, ilgili noktanın etrafındaki komşuluklar demektir.
# bir nokta komşularının yoğunluğundan daha düşük yoğunlukta ise o nokta daha seyrek bölgede olabilir,
# seyrek bölgede olabilir.
# 17, 3
...
clf = LocalOutlierFactor(n_neighbors=20) # komşuluk sayısı  ön tanımlı 20 tercik edilmeli
clf.fit_predict(df) # skorlar gertirir

df_scores = clf.negative_outlier_factor_ # takip edilebilirlik acısından
df_scores[0:5]
# df_scores = -df_scores # eksi değerlere göre değerlendirmek istemiyorsak. elbow yöntemi
np.sort(df_scores)[0:5] #en kötü 5 gözlem -1e yakın olanlar inline olarak değerlendirirler
# eşik değer belirlemek gerekir, veya programatik olarak yaparız
